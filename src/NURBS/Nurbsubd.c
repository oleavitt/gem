 ...
 */
static void FixNormals(SurfSample *s0, SurfSample *s1, SurfSample *s2)
{
	unsigned char goodnorm;
	long i, j, ok;
	double dist;
	SurfSample *V[3];
	Point3 norm;
	...
	for (ok = 0, goodnorm = FALSE;
		(ok < 3L) && !(goodnorm = (V[ok]->normLen > 0.0));
		ok++);
...
SplitSurface(NurbSurface * parent,
	...
	unsigned char dirflag)		/* If true subdivided in U, else in V */
{
	NurbSurface tmp;
	double * newkv;
	long i, j, splitPt;
...
IsCurveStraight(NurbSurface * n,
	double tolerance,
	long crvInd,
	unsigned char dirflag)  /* If true, test in U direction, else test in V */
{
	Point3 p, vec, prod;
	Point3 cp, e0;
	long i, last;
 ...
 */
static void
DoSubdivision(NurbSurface *n, double tolerance, unsigned char dirflag, long level)
{
