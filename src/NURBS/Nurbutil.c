...
int AllocNurb(NurbSurface *n, double *ukv, double *vkv)
{
	long i;

	if (ukv == NULL)
		n->kvU = (double *)malloc((n->numU + n->orderU) * sizeof(double));
	else
		n->kvU = ukv;
	if (vkv == NULL)
		n->kvV = (double *)malloc((n->numV + n->orderV) * sizeof(double));
	else
		n->kvV = vkv;

	n->points = (Point4 **)malloc((long)n->numV * (long)sizeof(Point4 *));
	if (n->points == NULL || n->kvV == NULL || n->kvU == NULL)
	{
		FreeNurb(n);
		return 0;
	}
	...
	for (i = 0; i < n->numV; i++)
	{
		n->points[i] = (Point4 *)malloc((long) n->numU * (long)sizeof(Point4));
		if (n->points[i] == NULL)
		{
			FreeNurb(n);
			return 0;
...
int CloneNurb(NurbSurface *src, NurbSurface *dst)
{
	long i, j;
	double *srcp, *dstp;
